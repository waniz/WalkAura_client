...
@export var border_color := Color(0.0, 0.0, 0.0, 0.4)

func _draw() -> void:
	var angle: float = (progress / max_value) * TAU
	if ring:
		# Border: draw a slightly larger and smaller ring behind
		if border_width > 0.0:
			draw_ring_arc(Vector2.ZERO, radius + border_width, radius - thickness - border_width, 0.0, TAU, border_color)
