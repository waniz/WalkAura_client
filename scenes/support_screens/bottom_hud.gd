...
const PAGE_LABELS := ["Profile", "Location", "Inventory", "", ""]
const PAGE_ICONS := [
...
func _apply_slot_style(slot: Dictionary, is_active: bool) -> void:
	...
	if is_active:
		var bg := Color(58.0 / 255.0, 46.0 / 255.0, 22.0 / 255.0, 1.0)
		var border := Color(220.0 / 255.0, 175.0 / 255.0, 68.0 / 255.0, 1.0)
		btn.add_theme_stylebox_override("normal", _make_sb(bg, border, true, 5))
		btn.add_theme_stylebox_override("hover", _make_sb(bg.lightened(0.08), border, true, 5))
		btn.add_theme_stylebox_override("pressed", _make_sb(bg.darkened(0.12), border, true, 5))
		btn.add_theme_stylebox_override("focus", _make_sb(bg, border, true, 5))
		btn.add_theme_stylebox_override("disabled", _make_sb(bg, border, true, 5))
		icon.modulate = Color.WHITE
		lbl.add_theme_color_override("font_color", Color(255.0 / 255.0, 210.0 / 255.0, 80.0 / 255.0, 1.0))
	elif is_nav:
		var bg := Color(28.0 / 255.0, 22.0 / 255.0, 14.0 / 255.0, 230.0 / 255.0)
		var border := Color(80.0 / 255.0, 64.0 / 255.0, 40.0 / 255.0, 180.0 / 255.0)
		btn.add_theme_stylebox_override("normal", _make_sb(bg, border, false, 0))
		btn.add_theme_stylebox_override("hover", _make_sb(bg.lightened(0.08), border, false, 0))
		btn.add_theme_stylebox_override("pressed", _make_sb(bg.darkened(0.12), border, false, 0))
		btn.add_theme_stylebox_override("focus", _make_sb(bg, border, false, 0))
		btn.add_theme_stylebox_override("disabled", _make_sb(bg, border, false, 0))
		...
		var bg := Color(20.0 / 255.0, 16.0 / 255.0, 12.0 / 255.0, 100.0 / 255.0)
		var border := Color(45.0 / 255.0, 36.0 / 255.0, 24.0 / 255.0, 100.0 / 255.0)
		btn.add_theme_stylebox_override("normal", _make_sb(bg, border, false, 0))
		btn.add_theme_stylebox_override("hover", _make_sb(bg, border, false, 0))
		btn.add_theme_stylebox_override("pressed", _make_sb(bg, border, false, 0))
		btn.add_theme_stylebox_override("focus", _make_sb(bg, border, false, 0))
		btn.add_theme_stylebox_override("disabled", _make_sb(bg, border, false, 0))
